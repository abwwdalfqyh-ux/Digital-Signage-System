...
const ScreenDetailPage = () => {
    ...
    return (
        <div className="p-6 md:p-8 max-w-7xl mx-auto w-full font-sans text-right" dir={dir} style={{ fontFamily: "'IBM Plex Sans Arabic', sans-serif" }}>
            ...
            {/* ══ Header ══ */}
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8 bg-white p-5 rounded-2xl border border-gray-200 shadow-sm">
                <div className="flex items-center gap-4">
                    <button onClick={() => navigate(-1)} className="flex items-center gap-2 bg-[#f1f3ff] hover:bg-[#2563eb] hover:text-white text-[#004ac6] border border-[#c3c6d7] px-4 py-2.5 rounded-xl font-bold transition-all shadow-sm cursor-pointer" title={t('common.back')}>
                        <ArrowRight className="w-5 h-5" />
                        <span className="text-sm font-bold">{t('common.back')}</span>
                    </button>
                    <div>
                        <div className="flex items-center gap-3">
                            <h1 className="text-3xl font-extrabold text-[#141b2b] m-0">{screen.screen_name}</h1>
                            <span className={`px-3 py-1 rounded-full text-sm font-bold ${isOnline ? 'bg-green-100 text-green-700 border border-green-300' : 'bg-red-100 text-red-700 border border-red-300'}`}>
                                {isOnline ? t('screens.status_online_now') : t('screens.status_offline_now')}
                            </span>
                        </div>
                    </div>
                </div>
                <div className="flex items-center gap-2 bg-[#004ac6] text-white px-5 py-2.5 rounded-xl font-bold text-base shadow-sm">
                    <Monitor className="w-5 h-5 text-white" />
                    <span>{t('screens.screen_id')}</span>
                    <span className="font-mono font-normal tracking-wider dir-ltr text-lg bg-white/20 px-2 py-0.5 rounded">{screen.mac_address}</span>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-1 space-y-6">
                    {/* Image Card */}
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white border text-center border-gray-200 rounded-2xl p-5 shadow-sm">
                        ...
                        <h3 className="font-bold text-[#141b2b] text-xl m-0 mb-1">{screen.screen_name}</h3>
                        <p className="text-base text-gray-500 font-normal m-0">{screen.street?.name || t('screens.unspecified_location')} {screen.street?.region?.name ? `- ${screen.street.region.name}` : ''}</p>
                    </motion.div>
                    ...
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm space-y-4">
                        <h4 className="font-bold text-[#141b2b] text-lg m-0 border-b pb-3">{t('screens.remote_control')}</h4>
                        
                        {screen.last_screenshot_url && (
                            <div className="mb-4">
                                <p className="text-sm font-bold text-gray-700 mb-2">{t('screens.last_screenshot')}</p>
                                ...
                                <p className="text-xs text-gray-500 font-light mt-1 text-left" dir="ltr">{new Date(screen.last_screenshot_at).toLocaleString()}</p>
                            </div>
                        )}
                        ...
                        <div className="grid grid-cols-2 gap-2.5">
                            <button onClick={() => handleCommand('TAKE_SCREENSHOT')} className="flex flex-col items-center justify-center gap-2 p-3 rounded-xl bg-blue-50 text-blue-700 hover:bg-blue-100 transition-colors border border-blue-100">
                                ...
                                <span className="text-sm font-bold">{t('screens.take_screenshot')}</span>
                            </button>
                            <button onClick={() => handleCommand('SLEEP_SCREEN')} className="flex flex-col items-center justify-center gap-2 p-3 rounded-xl bg-gray-50 text-gray-700 hover:bg-gray-100 transition-colors border border-gray-200">
                                ...
                                <span className="text-sm font-bold">{t('screens.turn_off_screen')}</span>
                            </button>
                            <button onClick={() => handleCommand('WAKE_SCREEN')} className="flex flex-col items-center justify-center gap-2 p-3 rounded-xl bg-yellow-50 text-yellow-700 hover:bg-yellow-100 transition-colors border border-yellow-100">
                                ...
                                <span className="text-sm font-bold">{t('screens.wake_up_screen')}</span>
                            </button>
                            <button onClick={() => handleCommand('RESTART_APP')} className="flex flex-col items-center justify-center gap-2 p-3 rounded-xl bg-red-50 text-red-700 hover:bg-red-100 transition-colors border border-red-100">
                                ...
                                <span className="text-sm font-bold">{t('screens.restart_screen')}</span>
                            </button>
                        </div>
                    </motion.div>
                    ...
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm space-y-4">
                        <h4 className="font-bold text-[#141b2b] text-lg m-0 border-b pb-3">{t('screens.custom_screen_performance')}</h4>
                        <div className="flex justify-between items-center">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-base font-bold text-gray-700">{t('screens.occupancy_today')}</span>
                            </div>
                            <span className="font-light text-2xl text-[#141b2b]">76%</span>
                        </div>
                        <div className="flex justify-between items-center">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-base font-bold text-gray-700">{t('screens.daily_impressions')}</span>
                            </div>
                            <span className="font-light text-2xl text-[#141b2b]">1,240</span>
                        </div>
                    </motion.div>
                </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
                        <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                            <div>
                                <h3 className="font-extrabold text-[#141b2b] text-xl m-0">{t('screens.active_ads_now')}</h3>
                            </div>
                            <span className="bg-blue-100 text-blue-700 px-3.5 py-1 rounded-full text-sm font-bold">{t('screens.n_active_campaigns').replace('{n}', 2)}</span>
                        </div>
                        <div className="p-0">
                            {/* Mocked Ads list */}
                            {[1, 2].map((ad, i) => (
                                <div key={i} className="flex items-center justify-between p-4 border-b border-gray-100 last:border-0 hover:bg-blue-50/30 transition-colors">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <h4 className="font-bold text-[#141b2b] text-base m-0">{t('screens.commercial_ad').replace('{ad}', ad)}</h4>
                                            <p className="text-sm text-gray-500 font-normal m-0 mt-0.5 max-w-[200px] truncate">{t('screens.modern_marketing_company')}</p>
                                        </div>
                                    </div>
                                    <div className="text-left">
                                        <div className="text-sm font-bold text-gray-700 bg-gray-100 px-3 py-1 rounded-lg">{t('screens.every_5_minutes')}</div>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    {/* Available Schedule - Subtitle DELETED */}
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
                        <div className="p-5 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
                            <div>
                                <h3 className="font-extrabold text-[#141b2b] text-xl m-0">{t('screens.screen_schedule_and_peak_times')}</h3>
                            </div>
                            <div>
                                <input 
                                    type="date" 
                                    ...
                                    className="border border-gray-300 rounded-lg px-3 py-1.5 text-base font-normal focus:ring-2 focus:ring-blue-500 outline-none"
                                />
                            </div>
                        </div>
                        <div className="p-5">
                            {loadingAvailability ? (
                                ...
                                <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                                    {availability.map((slot, i) => {
                                        ...
                                        return (
                                            <div key={i} className={`flex flex-col md:flex-row md:items-center gap-3 p-3.5 rounded-xl border ${slot.is_full ? 'border-red-200 bg-red-50/30' : 'border-gray-200 bg-white'} shadow-sm`}>
                                                <div className="flex flex-col justify-center md:w-56 flex-shrink-0">
                                                    <span className="font-bold text-[#141b2b] text-base" dir="rtl">{timeLabel}</span>
                                                ...
                                                <div className="flex-1">
                                                    <div className="flex justify-between text-sm mb-1">
                                                        <span className="text-gray-700 font-bold">{t('screens.reserved_seconds').replace('{seconds}', Math.round(3600 - slot.available_seconds))}</span>
                                                        <span className={slot.available_seconds > 0 ? 'text-green-700 font-bold' : 'text-red-700 font-bold'}>
                                                            {t('screens.remaining_seconds').replace('{seconds}', Math.round(slot.available_seconds))}
                                                        </span>
                                                    </div>
                                                    <div className="w-full bg-gray-100 rounded-full h-3 overflow-hidden flex">
                                                        <div className={`h-3 rounded-full ${slot.is_full ? 'bg-red-500' : 'bg-blue-600'}`} style={{ width: `${bookedPercentage}%` }}></div>
                                                    </div>
                                                </div>
                                            </div>
                                        );
